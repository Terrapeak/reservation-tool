...
async function cohortServicePage(business,service){
  ...
  shell(business,'<section class="booking-hero compact"><p class="booking-kicker">Continuing class</p><h1>'+esc(service.name)+'</h1><p>'+esc(service.description||'Enrol once to reserve a place in all future classes from your joining date.')+'</p><p><strong>'+remaining+' of '+service.capacity+' places available</strong></p><p>'+priceLabel(service)+'</p></section><section class="calendar-panel scheduled-calendar"><div><h2>Upcoming classes</h2><p>Your enrolment covers all future listed classes from your joining date. Individual classes may be postponed or cancelled by the centre.</p></div><div class="session-grid">'+sessionList+'</div></section>'+(remaining>0?'<form id="cohortEnrollmentForm" class="booking-form"><h2>Enrol in this class</h2><div class="form-grid"><label>Student name<input name="name" required maxlength="200"></label><label>Parent or contact email<input name="email" type="email" maxlength="320"></label><label>Phone<input name="phone" required maxlength="50"></label><label>Number of students<input name="quantity" type="number" min="1" max="'+remaining+'" value="1" required></label><label>Joining date<input name="joinsOn" type="date" min="'+dateValue(today)+'" value="'+dateValue(today)+'" required></label></div><label>Notes<textarea name="notes" maxlength="2000"></textarea></label><button class="booking-confirm" type="submit">Confirm enrolment</button><p id="cohortMessage" role="status"></p></form>':'<section class="booking-form"><h2>This class is full</h2><p>Please contact the centre about a waiting list.</p></section>'),crumb)
  const form=document.querySelector('#cohortEnrollmentForm');if(!form)return
  form.onsubmit=async event=>{event.preventDefault();const values=new FormData(form),button=form.querySelector('[type="submit"]'),message=document.querySelector('#cohortMessage');button.disabled=true;message.textContent='Confirming enrolment…';const {data,error}=await supabase.rpc('create_public_class_enrollment',{p_business_slug:business.business_slug,p_service_slug:service.slug,p_customer_name:values.get('name'),p_customer_email:values.get('email')||null,p_customer_phone:values.get('phone'),p_quantity:Number(values.get('quantity')),p_joins_on:values.get('joinsOn'),p_notes:values.get('notes')||null});button.disabled=false;if(error){message.textContent=error.message.includes('enough places')?'This class no longer has enough places. Please refresh and try again.':'Enrolment could not be completed.';return}form.innerHTML='<div class="booking-success"><p class="booking-kicker">Enrolment confirmed</p><h2>Welcome, '+esc(values.get('name'))+'.</h2><p>Your place is reserved in future '+esc(service.name)+' classes from '+esc(values.get('joinsOn'))+'.</p><p>Your reference is <strong>'+esc(data[0].reference)+'</strong>.</p><p>'+data[0].remaining+' place'+(data[0].remaining===1?'':'s')+' remain.</p></div>'}
}
