...
async function installCustomerForm() {
  const { data: fields = [], error } = await supabase.rpc('get_public_booking_custom_fields', {
...
function renderField(field, name) {
  ...
  if (field.field_type === 'textarea') {
    return `<label>${label}<textarea name="${name}" maxlength="2000"${required}></textarea></label>`
  }
  ...
  if (field.field_type === 'checkbox') {
    return `<label class="booking-checkbox"><input type="checkbox" name="${name}"${required}> ${label}</label>`
  }
