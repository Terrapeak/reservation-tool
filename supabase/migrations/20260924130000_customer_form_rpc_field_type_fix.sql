-- Restore the canonical Customer Form field-type contract in the ownership-safe save.
-- Existing provenance and protected-row semantics remain database-owned.
...
begin
  ...
  loop
    ...
    if requested_type not in ('text', 'textarea', 'dropdown', 'checkbox', 'email', 'phone', 'number', 'date') then
    ...
    if requested_id is not null then
      ...
      -- Provenance is never taken from the client payload. Template/system rows
      -- retain their metadata and active state; only display order is editable.
      if existing.field_source in ('template', 'system') or existing.is_locked then
        if existing.is_locked
          and (existing.field_label is distinct from requested_label
  ...
  -- Only customer/legacy rows participate in ordinary UI removal cleanup.
  -- Template/system rows cannot be deactivated or deleted by this RPC.
  update public.booking_custom_fields
  set is_active = false
  where business_id = p_business_id
    and field_source in ('customer', 'legacy')
